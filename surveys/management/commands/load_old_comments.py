...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        file_path = os.path.join(settings.MASTER_DIR, 'old_comments.csv')
        ...
        for index, row in comments.iterrows():
            ...
            meta_objects = MetaObject.objects.filter(Q(RA__gt=(ra_r-0.5)), Q(RA__lt=(ra_r+0.5)), Q(DEC__gt=(dec_r-0.5)), Q(DEC__lt=(dec_r+0.5)))
            if not meta_objects.exists():
                print(f"\n {index} No Meta Objects for comment: {row['comment']} ra: {row['RA']}, dec: {row['DEC']}\n")
            else:
                print(f'\n{index} - {meta_objects}')
                meta_ra = meta_objects.values_list('RA', flat=True)
                meta_dec = meta_objects.values_list('DEC', flat=True)
                ...
                if seps_30.size == 0:
                    print(f"\nNo Meta Objects in 30'' for comment: {row['comment']} ra: {row['RA']}, dec: {row['DEC']}\n")
                else:
                    print(f'\nSeps less than 30": {seps_30}')
                    # get meta objects in 30 arcsec radius
                    sep_indexes = np.where(seps < 30)[0]
                    # get meta objects by indexes
                    near_objs = meta_object_arr[sep_indexes]
                    print(f'Near meta objects: {near_objs}')
                    nearest_obj = meta_object_arr[np.argmin(seps)]
                    print(f'Nearest object: {nearest_obj}')
                    if not nearest_obj.primary_object:
                        group = nearest_obj.meta_group
                        nearest_obj = group.meta_objects.get(primary_object=True)
                        print(f'Nearest primary object: {nearest_obj}')

                    # create comment
                    print(f"Archive comment: {row['comment']}")
                    self.stdout.write(f"NOTE:Create comment for object {nearest_obj} by {archive_user}")
                    comment = Comment.objects.create(comment=row['comment'], created_by=archive_user, meta_source=nearest_obj)
